...
│   ├── config.js           # Public extension configuration
...
The extension uses a separate `config.js` file for public configuration values.

Example structure:

```javascript
const THM_BADGE_CONFIG = {
    githubClientId: 'YOUR_GITHUB_APP_CLIENT_ID',
...
`config.js` may contain the **GitHub App Client ID**, because a Client ID is not a secret.

**Never put the following in `config.js`:**
...
The GitHub Client Secret must remain on the OAuth backend and must never be distributed with the Firefox extension.
...
### 2. Install or manage the GitHub App
...
and grant the app access to the repository you want to use.
...
- `config.js` contains the correct public configuration
...
### `THM_BADGE_CONFIG is not defined`

Make sure:

1. `config.js` exists inside the `extension/` directory.
2. `popup.html` loads `config.js` before `popup.js`.

The scripts should appear in this order:
